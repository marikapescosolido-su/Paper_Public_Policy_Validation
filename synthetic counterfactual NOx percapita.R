...
##############################
# 2. Load Raw NOx Panel (wide) and reshape to long
##############################
library(dplyr)
library(tidyr)
library(lubridate)
...
df_full <- read.csv(
  "C:/Users/daga1/OneDrive/Documents/PUBP3042 Project/EDGAR_NOx_m_2000_2022/NOx_monthly_percapita_wide.csv",
...
# 1) Wide -> Long (per-capita NOx)
df_full <- df_full %>%
  tidyr::pivot_longer(
    cols = all_of(month_cols_pc),
    names_to = "MonthCol",
    values_to = "nox"        # still call it 'nox' so the rest of the code doesn't change
  ) %>%
  mutate(
...
df_full <- df_full %>%
  ...
  summarise(nox = mean(nox, na.rm = TRUE), .groups = "drop") %>%
...
panel_wide <- df_full %>%
  ...
  select(country, year_month, date, nox) %>%
  tidyr::pivot_wider(
...
ggplot(results, aes(x = date)) +
  ...
  labs(
    title = "Synthetic Control: Pakistan NOx per capita (Full Period)",
    x = "Date",
    y = "NOx per capita (kg/person/month)",
    colour = ""
  ) +
...
ggplot(results, aes(x = date)) +
  ...
  labs(
    title = "STL-Adjusted Synthetic Control: Pakistan NOx per capita",
    x = "Date",
    y = "NOx per capita (kg/person/month, seasonally adjusted)",
    colour = ""
  ) +
  theme_minimal(base_size = 14)
...
ggplot(results, aes(x = date, y = gap)) +
  ...
  labs(
    title = "Euro-V Effect on Pakistan NOx per capita (Actual - Synthetic)",
    x = "Date",
    y = "NOx per capita difference (kg/person/month)"
  ) +
  theme_minimal(base_size = 14)
...
ggplot(results, aes(x = date, y = gap_stl)) +
  ...
  geom_hline(yintercept = 0, linetype = "dashed") +
  labs(
    title = "Euro-V Effect on Pakistan NOx per capita (STL-Adjusted)",
    x = "Date",
...
did_twfe <- feols(
  nox ~ treated * post | unit_id + time_id,
  data = did_df
)
summary(did_twfe)
