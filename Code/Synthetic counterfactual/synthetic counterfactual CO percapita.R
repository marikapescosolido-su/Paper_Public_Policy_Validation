...
##############################
# 2. Load Raw CO Panel (wide) and reshape to long
##############################
library(dplyr)
library(tidyr)
library(lubridate)
...
df_full <- read.csv(
  "C:/Users/daga1/OneDrive/Documents/PUBP3042 Project/EDGAR_CO_m_2000_2022/CO_monthly_percapita_wide.csv",
...
# 1) Wide -> Long (per-capita CO)
df_full <- df_full %>%
  tidyr::pivot_longer(
    cols = all_of(month_cols_pc),
    names_to = "MonthCol",
    values_to = "co"        # still call it 'co' so the rest of the code doesn't change
  ) %>%
  mutate(
...
df_full <- df_full %>%
  ...
  summarise(co = mean(co, na.rm = TRUE), .groups = "drop") %>%
...
panel_wide <- df_full %>%
  ...
  select(country, year_month, date, co) %>%
  tidyr::pivot_wider(
...
ggplot(results, aes(x = date)) +
  ...
  labs(
    title = "Synthetic Control: Pakistan CO per capita (Full Period)",
    x = "Date",
    y = "CO per capita (kg/person/month)",
    colour = ""
  ) +
...
ggplot(results, aes(x = date)) +
  ...
  labs(
    title = "STL-Adjusted Synthetic Control: Pakistan CO per capita",
    x = "Date",
    y = "CO per capita (kg/person/month, seasonally adjusted)",
    colour = ""
  ) +
  theme_minimal(base_size = 14)
...
ggplot(results, aes(x = date, y = gap)) +
  ...
  labs(
    title = "Euro-V Effect on Pakistan CO per capita (Actual - Synthetic)",
    x = "Date",
    y = "CO per capita difference (kg/person/month)"
  ) +
  theme_minimal(base_size = 14)
...
ggplot(results, aes(x = date, y = gap_stl)) +
  ...
  geom_hline(yintercept = 0, linetype = "dashed") +
  labs(
    title = "Euro-V Effect on Pakistan CO per capita (STL-Adjusted)",
    x = "Date",
...
did_twfe <- feols(
  co ~ treated * post | unit_id + time_id,
  data = did_df
)
summary(did_twfe)
